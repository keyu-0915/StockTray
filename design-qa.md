...
- Brand: the header, footer, application title bar, and README use the approved scheme 1 two-color `StockTray` wordmark (`Stock` neutral, `Tray` brand blue). The window/taskbar, website favicon, and installer resources use the separate scheme 2 two-color `ST` icon (`S` neutral, `T` brand blue). The system tray intentionally keeps its dynamic profit/loss arrows so glanceable market status is not lost.
